...
(function () {
  ...
  try {
    ...
    window.fetch = function (input, init) {
      ...
      function attempt(n) {
        return original.apply(self, args).then(function (res) {
          return res.clone().text().then(function (text) {
            if (looksLikeJson(text) || n >= CONFIG.MAX_ATTEMPTS) return rebuild(text, res);
          ...
          if (n >= CONFIG.MAX_ATTEMPTS) throw err;
          return wait(CONFIG.DELAY_MS).then(function () { return attempt(n + 1); });
        });
      }
